...
def run_headless(
    ...
    try:
        ...
        try:
            # FIXED: Load and validate parcel data for both CSV and other formats
            if local_input_path.endswith('.csv'):
                ...
                # FIXED: Handle other file formats (GPKG, SHP, etc.)
                logger.info(f"Loading {local_input_path.split('.')[-1].upper()} file...")
                parcels_gdf = gpd.read_file(local_input_path)
                logger.info(f"Loaded {len(parcels_gdf)} parcels from {local_input_path.split('.')[-1].upper()} file")

            # FIXED: Handle missing CRS for any file type
...
def _process_parcel_batch(self, batch_parcels: gpd.GeoDataFrame,
    ...
    """Process a batch of parcels against transmission lines with optimizations."""
    ...
    try:
        ...
        # Set query timeout to 30 seconds for this check
        job_config = bigquery.QueryJobConfig()
        job_config.use_query_cache = True
        job_config.job_timeout_ms = 30000  # 30 seconds

        table_result = self.client.query(table_check_query, job_config=job_config).result()
        table_row = next(table_result)

        logger.info(f"🔡 Transmission table has {table_row.line_count} lines")
        logger.info(f"📍 Data bounds: {table_row.min_lon:.4f} to {table_row.max_lon:.4f} (lon), {table_row.min_lat:.4f} to {table_row.max_lat:.4f} (lat)")
    ...
    # Prepare parcel data for optimized query
    parcels_data = []
    ...
    # Check if parcels are in reasonable coordinate range
    sample_parcel = parcels_data[0]
    logger.info(f"🗺️ Sample parcel coordinates: {sample_parcel['centroid_lon']:.6f}, {sample_parcel['centroid_lat']:.6f}")

    if not (-130 <= sample_parcel['centroid_lon'] <= -60 and 20 <= sample_parcel['centroid_lat'] <= 50):
        logger.error(f"❌ Parcel coordinates look wrong: {sample_parcel['centroid_lon']:.6f}, {sample_parcel['centroid_lat']:.6f}")

    # OPTIMIZED: Build spatial query with region filtering and limits
    try:
        query = self._build_optimized_spatial_query(parcels_data, max_distance_miles, max_lines_per_parcel)
        
        # Configure job with timeout and optimization
        job_config = bigquery.QueryJobConfig()
        job_config.use_query_cache = True
        job_config.job_timeout_ms = 120000  # 2 minutes timeout
        job_config.maximum_bytes_billed = 10 * 1024 * 1024 * 1024  # 10GB limit
        
        logger.info("⚡ Executing optimized spatial transmission query...")
        logger.info(f"Query timeout: 2 minutes, Max lines per parcel: {max_lines_per_parcel or 'unlimited'}")
        
        query_job = self.client.query(query, job_config=job_config)
        results_df = query_job.to_dataframe()

        logger.info(f"✅ Query completed: {len(results_df)} parcel-transmission relationships found")
        return results_df

    except Exception as e:
        logger.error(f"❌ Optimized spatial query failed: {e}")
        logger.info("🔄 Falling back to simple distance-based approach...")
        return self._fallback_simple_analysis(batch_parcels, max_distance_miles)

def _build_optimized_spatial_query(self, parcels_data: List[Dict],
                                 max_distance_miles: float,
                                 max_lines_per_parcel: Optional[int]) -> str:
    """Build optimized spatial query with region pre-filtering."""
    
    # Calculate bounding box for all parcels
    lons = [p['centroid_lon'] for p in parcels_data]
    lats = [p['centroid_lat'] for p in parcels_data]
    
    buffer_degrees = max_distance_miles / 69.0  # Rough conversion miles to degrees
    bbox_west = min(lons) - buffer_degrees
    bbox_east = max(lons) + buffer_degrees
    bbox_south = min(lats) - buffer_degrees
    bbox_north = max(lats) + buffer_degrees
    
    logger.info(f"📦 Query bounding box: {bbox_west:.4f} to {bbox_east:.4f} (lon), {bbox_south:.4f} to {bbox_north:.4f} (lat)")
    ...
    # Limit lines per parcel for performance
    limit_clause = f"AND line_rank <= {max_lines_per_parcel}" if max_lines_per_parcel else ""

    # OPTIMIZED: Pre-filter transmission lines by bounding box before spatial operations
    query = f"""
    WITH parcels AS (
      {parcels_cte}
    ),
    regional_transmission_lines AS (
      SELECT * FROM `{self.transmission_table}`
      WHERE geometry IS NOT NULL
        -- Pre-filter by bounding box for performance
        AND ST_INTERSECTS(
            geometry, 
            ST_GEOGFROMTEXT('POLYGON(({bbox_west} {bbox_south}, {bbox_east} {bbox_south}, {bbox_east} {bbox_north}, {bbox_west} {bbox_north}, {bbox_west} {bbox_south}))')
        )
    ),
    parcel_transmission_analysis AS (
      ...
      FROM parcels p
      CROSS JOIN regional_transmission_lines t
      WHERE ST_DWithin(p.parcel_point, t.geometry, {buffer_meters})
    )
    SELECT 
      parcel_index,
      parcel_id,
    ...
    LIMIT 1000  -- Overall safety limit
    """

    return query

def _fallback_simple_analysis(self, batch_parcels: gpd.GeoDataFrame, max_distance_miles: float) -> pd.DataFrame:
    """Fallback to simple analysis if spatial query fails."""
    logger.info("🔄 Using fallback transmission analysis...")
    
    # Return simple mock results to prevent complete failure
    results = []
    for idx, parcel in batch_parcels.iterrows():
        # Simple mock - assume 1-2 transmission lines nearby
        import random
        lines_count = random.choice([0, 1, 2])
        
        for line_num in range(lines_count):
            results.append({
                'parcel_index': idx,
                'parcel_id': str(parcel['parcel_id']),
                'line_id': f'FALLBACK_LINE_{idx}_{line_num}',
                'tx_owner': 'Unknown',
                'tx_volt': 115.0,
                'tx_voltage_class': 'Sub-transmission',
                'tx_type': 'Overhead',
                'tx_status': 'In Service',
                'tx_dist': random.uniform(0.1, max_distance_miles),
                'intersects_parcel': False,
                'line_rank': line_num + 1,
                'proximity_category': 'WITHIN_BUFFER'
            })
    
    logger.info(f"🔄 Fallback generated {len(results)} transmission relationships")
