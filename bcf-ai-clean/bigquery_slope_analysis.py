...
def load_parcel_data(file_path: str) -> Optional[gpd.GeoDataFrame]:
    ...
    try:
        ...
        # FIXED: Handle CSV files with WKT geometry columns
        if file_path.endswith('.csv'):
            ...
            if geometry_col:
                ...
                # Handle potential invalid geometries
                valid_geometries = []
                for idx, geom_text in enumerate(df[geometry_col]):
                    try:
                        if pd.isna(geom_text) or geom_text == '':
                            logger.warning(f"Row {idx}: Empty geometry, skipping")
                            continue
                        geom = wkt.loads(str(geom_text))
                        valid_geometries.append((idx, geom))
                    except Exception as e:
                        logger.warning(f"Row {idx}: Invalid geometry '{geom_text}', skipping: {e}")
                        continue
                
                if not valid_geometries:
                    raise ValueError("No valid geometries found in CSV file")
                
                # Filter dataframe to only include rows with valid geometries
                valid_indices = [idx for idx, _ in valid_geometries]
                df_filtered = df.iloc[valid_indices].copy()
                df_filtered['geometry'] = [geom for _, geom in valid_geometries]
                
                gdf = gpd.GeoDataFrame(df_filtered, geometry='geometry')
                gdf = gdf.set_crs('EPSG:4326')
                logger.info(f"Created GeoDataFrame with {len(gdf)} valid geometries, CRS: {gdf.crs}")
            else:
                logger.error(f"No geometry column found. Available columns: {list(df.columns)}")
                raise ValueError(f'No geometry column found. Available: {list(df.columns)}')
        else:
            # Handle other file formats (GPKG, SHP, etc.)
